...
def train_evaluate_model(model, X_train: pd.DataFrame, y_train: pd.Series, X_test: pd.DataFrame=None, y_test: pd.Series=None,
    ...
    if save_path is not None:
        joblib.dump(model, save_path)       # save the model
...
def store_fasterrisk_cards(model: FasterRisk, card_path: str, X_train: pd.DataFrame, X_test: pd.DataFrame, 
                           y_train: pd.Series, y_test: pd.Series, names: list, K: int=1, card_title: str='Risk Score Card'):
    print(f"\nStoring {K} FasterRisk Risk Card(s)...")
    terminal = sys.stdout
    sys.stdout = open(f"{card_path}.txt", "wt")
    if K is not None:
    ...
    sys.stdout = terminal
    print(f".txt risk score card(s) saved at \"{card_path}.txt\"")
    
    file =  open(f"{card_path}.txt", "r")
    prints = file.read()
    model.visualize_risk_card(prints, score_card_name='', title=card_title, save_path=f'{card_path}.png')
    print(f"Risk score card image saved at \"{card_path}.png\"")