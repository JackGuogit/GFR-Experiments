...
if __name__ == "__main__":
    ...
    prs.add_argument("--data", dest='data', type=str, default='mimic')
    args, unknown = prs.parse_known_args()
    
    # ----------------------------------------- DATA SOURCE -------------------------------------------
    if args.fold is not None:       # if using folds, do fold on MIMIC-III dataset
        args.path = f"src/exp_6.6_to_6.27/data/k-fold/TRAIN-union-features-fold{args.fold}.csv"
        train = pd.read_csv(args.path)
    elif args.data == 'mimic':      # use entire MIMIC-III dataset to tune parameter for out of distribution evaluation
        args.path = f"src/exp_6.6_to_6.27/data/TRAIN-union-features.csv\nsrc/exp_6.6_to_6.27/data/TEST-union-features.csv"
    ...
    elif args.data == 'eicu':       # use eicu dataset to tune
        args.path = "src/exp_6.6_to_6.27/data/eICU-union.csv"
        train = pd.read_csv(args.path)
        train = train.drop(['uniquepid', 'patientunitstayid', 'apache_iv_prob', 'apache_iva_prob', 'oasis_prob', 'sapsii_prob'], axis=1)
